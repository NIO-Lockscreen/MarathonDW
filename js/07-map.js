...
function w2m(x,z){ // world -> map px. world x -50..50, z -50..95
...
function drawMap(){
  ...
  SPAWNS.forEach((s,i)=>{
    ...
    const found = switches.find(sw=>sw.idx===i && sw.dead);
    g.beginPath(); g.arc(x,y,5,0,7);
    g.strokeStyle = found? '#c8ff00' : '#c8ff00';
    if(found){ g.fillStyle='#c8ff00'; g.fill(); }
    g.stroke();
    g.fillStyle='#9fc'; g.font='8px monospace';
    g.fillText(String(i+1), x+7, y+3);
  });
  // player
